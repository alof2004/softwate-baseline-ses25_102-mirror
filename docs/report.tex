...
\begin{figure}[H]
  ...
  \caption{AS-06 -- Destroy Patient Records via Cascade Deletion: \textbf{Residual Risk}}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{07_regression.png}
  \caption{AS-07 -- Undetectable Actions: \textbf{Residual Risk}; AS-08 -- API Overload: \textbf{Prevented}}
\end{figure}
...
\midrule
Prevented      & 6 & AS-01, AS-02, AS-03, AS-04, AS-05, AS-08 \\
Residual Risk  & 2 & AS-06, AS-07 \\
Detected only  & 0 & -- \\
\bottomrule
\end{tabular}
\end{table}

The two residual risks share a common theme: the absence of persistent, structured observability.
AS-06 requires a soft-delete mechanism to prevent irreversible data loss; AS-07 requires a
structured audit log to make actor actions investigable. Both are identified as future work items.

% ─────────────────────────────────────────────────────────────────────────────
\section{Future Work}
% ─────────────────────────────────────────────────────────────────────────────

Several security improvements remain identified but not yet implemented.
A \textbf{structured audit log} should persist actor identity, timestamp, resource, and action
for every CRUD operation; this is required for GDPR compliance and forensic investigation
(ZTA tenet 7, AS-07). Closely related, a \textbf{soft-delete} mechanism should replace cascade
hard deletion with a \texttt{deletedAt} timestamp to prevent irreversible data loss (AS-06).

On the data-model side, replacing sequential integer primary keys with \textbf{UUID primary keys}
would eliminate the ID enumeration vector exploited in AS-03, making brute-force IDOR
significantly harder even for authenticated callers. Regarding transport security,
\textbf{mTLS for inter-service communication} should extend TLS from the current WAF perimeter
to internal container-to-container channels (WAF $\to$ backend, backend $\to$ Keycloak) to fully
satisfy ZTA tenet 2. Finally, \textbf{device posture} assessment -- integrating endpoint verification
before granting access -- is needed to address ZTA tenet 1.
...
Of the eight abuse stories from Part 1, six are now classified as \textbf{Prevented} and two
remain \textbf{Residual Risk}. AS-06 (cascade deletion by a compromised admin) is mitigated by
role restriction but not fully resolved without a soft-delete mechanism. AS-07 (undetectable
actions) is partially addressed by JWT actor identity but lacks a persistent audit log. These
two items, together with UUID primary keys and full mTLS coverage, represent the clearest
priorities for any future security iteration of the system.
