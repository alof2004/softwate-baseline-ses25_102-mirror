...
\begin{tabularx}{\linewidth}{S{0.8cm} S{3.5cm} S{2.6cm} S{1.7cm} X}
\toprule
...
S-02 & Attacker can forge \texttt{patientId} query parameter to create appointments under any patient. & POST /api/\allowbreak appointments & \high & \texttt{AppointmentController.create()} accepts \texttt{patientId} from the request. \texttt{AppointmentService.create()} trusts it after \texttt{findById()}. \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{ID} & \textbf{Threat} & \textbf{Component} & \textbf{Severity} & \textbf{Detail} \\
\midrule
T-01 & Any unauthenticated user can PUT to \texttt{/api/appointments/\{id\}} and change patient linkage, date, status, or specialty. & Appointment API & \critical & \texttt{AppointmentService.update()} updates all mutable fields and can swap the linked patient. \\[6pt]
T-02 & Any unauthenticated user can PUT to \texttt{/api/patients/\{id\}} and modify patient PII. & Patient API & \critical & \texttt{PatientService.update()} directly replaces name, dateOfBirth, phoneNumber, and email. \\[6pt]
...
T-04 & SPECIALTIES and STATUSES not validated server-side -- arbitrary strings can be persisted. & Appointment entity & \medium & \texttt{specialty} and \texttt{status} are plain Strings with no enum or bean validation. \\[6pt]
T-05 & Email field has no uniqueness constraint enforced by the application. & Patient API & \medium & \texttt{findByEmail()} exists in the repository but is not used during create or update flows. \\
\bottomrule
\end{tabularx}
\end{table}
...
D-02 & No pagination on \texttt{GET /api/appointments} -- large result sets can exhaust memory. & Appointment search & \medium & \texttt{appointmentRepository.findAll(spec)} returns the full matching set in memory. \\[6pt]
...
E-03 & \texttt{PUT /api/appointments/\{id\}} re-assigns patient -- allows transferring medical records between patients. & Appointment service & \high & \texttt{AppointmentService.update()} resolves the provided patient ID and rebinds the appointment if found. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[LINDDUN] Disclosure -- \critical
  \item[Abuse Case] As a malicious external user, I want to access another patient's personal data so that I can obtain sensitive information such as name, email, phone number, date of birth, and linked appointment data.
  \item[How] The attacker sends direct requests to \texttt{/api/patients} and \texttt{/api/appointments} and retrieves records without any access control, as no authentication mechanism exists in the system.
  \item[Impact] Exposure of personal and appointment-related data, leading to a privacy breach.
\end{description}

\subsection*{AS-02 -- Link Patient Identity to Appointment History}
\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[LINDDUN] Linking -- \critical
  \item[Abuse Case] As a malicious external user, I want to associate patient identity data with appointment records so that I can infer sensitive patterns such as recurring specialties and possible health-related conditions.
  \item[How] The attacker correlates the full patient list with appointment records through linked identifiers. Both endpoints are unauthenticated and return all records without pagination, making full profiling possible in two requests.
  \item[Impact] Sensitive health patterns can be inferred from metadata alone, increasing patient profiling risk beyond what any single record reveals.
\end{description}

\subsection*{AS-03 -- Directly Identify a Patient from Stored Records}
\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[LINDDUN] Identifying -- \critical
  \item[Abuse Case] As a malicious external user, I want to use personal identifiers stored in the system so that I can directly identify a patient and associate their appointment information with a real individual.
  \item[How] The attacker accesses records containing direct identifiers (name, email, phone, date of birth). Patient IDs are sequential integers starting at 1, making full enumeration via \texttt{/api/patients/\{id\}} trivial with no rate limiting or authentication.
  \item[Impact] A real individual can be directly identified and their full appointment history retrieved.
\end{description}

\subsection*{AS-04 -- Infer a Patient's Health Condition from Appointment Metadata}
\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[LINDDUN] Detecting -- \critical
  \item[Abuse Case] As a malicious external user, I want to observe the specialty and status fields of a patient's appointments so that I can infer sensitive health information without ever accessing a medical record directly.
  \item[How] The attacker filters \texttt{GET /api/appointments?patientName=X} to retrieve the full appointment history. Repeated ``Psychiatry'' visits with status ``NO\_SHOW'' reveal sensitive patterns without any explicit diagnosis data.
  \item[Impact] Sensitive health conditions may be inferred from observable metadata, constituting a privacy violation.
\end{description}

\subsection*{AS-05 -- Modify or Reassign Patient Appointment Records}
\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[LINDDUN] Disclosure -- \high
  \item[Abuse Case] As a malicious external user, I want to change the date, status, specialty, or linked patient of an appointment so that I can corrupt legitimate medical records.
  \item[How] The attacker sends an unauthenticated PUT request to \texttt{/api/appointments/\{id\}}. Including \texttt{"patient": \{"id": X\}} in the body causes the service to silently reassign the appointment to a different patient, with no authorisation check, no validation, and no audit log.
  \item[Impact] Medical records may be altered or linked to the wrong patient, directly affecting clinical integrity and patient safety.
\end{description}

\subsection*{AS-06 -- Destroy Patient Records Through Cascade Deletion}
\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[LINDDUN] Non-Compliance -- \critical
  \item[Abuse Case] As a malicious external user, I want to remove patient records so that I can cause irreversible data loss and disrupt normal clinical operation.
  \item[How] The attacker sends unauthenticated DELETE requests to \texttt{/api/patients/\{id\}}. Due to cascade deletion (\texttt{orphanRemoval = true}), every linked appointment is also permanently destroyed. IDs are sequential, so iterating from 1 to N wipes the entire database. No confirmation, audit log, or recovery path exists.
  \item[Impact] All patient PII and linked appointment history is permanently lost with no recovery mechanism. The clinic cannot detect, investigate, or report the incident.
\end{description}

\subsection*{AS-07 -- Perform Undetectable Actions Due to Absence of Audit Logging}
\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[LINDDUN] Non-repudiation -- \critical
  \item[Abuse Case] As a malicious internal user or external attacker, I want to create, modify, or delete patient and appointment records so that my actions leave no trace and cannot be attributed to me.
  \item[How] The system has no authentication, no audit log, and no access log at any layer. Any CRUD operation on any record is permanently undetectable once performed.
  \item[Impact] Malicious or accidental changes cannot be investigated or attributed. The clinic cannot comply with GDPR.
\end{description}

\subsection*{AS-08 -- Overload the API with Repeated Requests}
\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[LINDDUN] Non-compliance -- \high
  \item[Abuse Case] As an attacker, I want to flood patient and appointment endpoints with repeated requests so that I can make the system slow or unavailable for legitimate users.
  \item[How] The attacker automates repeated API requests. No rate limiting exists, and \texttt{GET /api/appointments} with no filters triggers a full table scan with no pagination, exhausting the HikariCP connection pool ($\sim$10 connections default).
  \item[Impact] The system becomes slow or unavailable for normal clinical use.
\end{description}
...
\begin{itemize}
  \item \textbf{Semgrep (medium)} -- dynamic URL used with \texttt{urllib} in \texttt{.github/scripts/build\_sonarqube\_pr\_comment.py:24}.
  \item \textbf{ESLint (2 warnings)} -- Generic Object Injection Sink in \texttt{AppointmentsPanel.jsx:145} and \texttt{clinicOptions.js:32}.
...
\begin{enumerate}
  ...
  \item The frontend calls \texttt{keycloak.logout(\{redirectUri: window.location.origin\})}.
...
\begin{enumerate}
  \item Before every API request, \texttt{httpConsumer.js} calls
        \texttt{await keycloak.updateToken(30)}.
...
\begin{description}
  ...
  \item[\textbf{Method-level}] Every controller method is annotated with
    \texttt{@PreAuthorize("@perms.canAnyRole(authentication, '<resource>', '<action>')")}.
    The \texttt{@perms} bean ({\small\texttt{PermissionsConfig}}) checks the caller's
...
\begin{longtable}{S{1.0cm} S{2.2cm} S{5.2cm} S{5.2cm}}
...
\texttt{RateLimitFilter} (token-bucket, 60 req/min per IP) registered on all \texttt{/api/**} routes. Request-size limits set to 1\,MB (\texttt{server.tomcat.max-http-form-post-size}, \texttt{spring.servlet.multipart.max-request-size}). \\[6pt]

API5 & Function-Level Auth & All endpoints accessible to any unauthenticated caller; no deny-by-default. &
\texttt{SecurityConfig} uses \texttt{anyRequest().denyAll()} as the catch-all. All \texttt{/api/**} requires authentication. No route is reachable without a valid JWT. \\[6pt]

API8 & Security Misconfiguration & No security headers; no CSRF/frame protection; stack traces in error responses. &
\texttt{SecurityConfig} sets HSTS (\texttt{max-age=31536000; includeSubDomains}), \texttt{X-Frame-Options: DENY}, \texttt{X-Content-Type-Options}, and \texttt{Content-Security-Policy: default-src 'none'; frame-ancestors 'none'}. Form login and HTTP Basic are disabled. \\[6pt]

API9 & Improper Inventory & No API documentation or endpoint inventory. &
\texttt{springdoc-openapi} generates an OpenAPI 3 specification at \texttt{/v3/api-docs}. Swagger UI is served at \texttt{/swagger-ui.html}. Both are \texttt{permitAll()} (public documentation). \texttt{@Operation} and \texttt{@Tag} annotations document each endpoint. \\
\end{longtable}
...
\textbf{Policy Enforcement Point (PEP)} &
  Two layers: (1) \texttt{SecurityFilterChain} -- enforces JWT presence on every
  \texttt{/api/**} request; (2) Spring \texttt{@PreAuthorize} annotations on each
  controller method -- enforce the role $\times$ resource $\times$ action permission. \\[6pt]
...
2 & All communication is secured regardless of network location. &
  \textbf{Partial} &
  All API communication is bearer-token authenticated (JWT RS256). TLS is not configured in the development Docker Compose setup; production deployment would require TLS termination at the reverse proxy. \\[6pt]
...
AS-06 & Destroy Patient Records via Cascade Deletion &
  \textbf{\textcolor{sevmedium}{Residual Risk}} &
  \texttt{DELETE /api/patients/\{id\}} now requires authentication and the ADMIN DELETE role, blocking external attackers. However, cascade deletion (\texttt{CascadeType.ALL + orphanRemoval}) remains in place -- a compromised admin account can still destroy all linked records irreversibly. No soft-delete or confirmation guard exists. \\[8pt]

AS-07 & Perform Undetectable Actions &
  \textbf{\textcolor{sevmedium}{Residual Risk}} &
  Authentication now records actor identity (JWT \texttt{sub} claim) with every request, partially addressing R-02. However, no structured audit log persists CRUD operations. Actions performed by an authenticated user remain uninvestigable after the fact. \\[8pt]

AS-08 & Overload the API with Repeated Requests &
  \textbf{\textcolor{sevmedium}{Residual Risk}} &
  \texttt{RateLimitFilter} limits each IP to 60 requests/min, significantly raising the cost of a DoS attack. Authentication adds a further barrier. However, no WAF, IP-based throttling at the network perimeter, or connection pool tuning has been applied. A distributed attack (multiple IPs) remains feasible. \\
\end{longtable}
...
The three residual risks share a common root cause: the absence of a structured audit log
(AS-07) and a WAF/perimeter layer (AS-08) and a soft-delete mechanism (AS-06). These are
identified as future work items.
...
\begin{enumerate}
  \item \textbf{Structured audit log} -- persist actor identity, timestamp, resource,
        and action for every CRUD operation. Required for GDPR compliance and forensic
        investigation (ZTA tenet 7, AS-07).
  \item \textbf{Soft delete} -- replace cascade hard deletion with a
        \texttt{deletedAt} timestamp to prevent irreversible data loss (AS-06).
  \item \textbf{Perimeter WAF} -- place Nginx + ModSecurity (OWASP Core Rule Set)
        in front of the application for IP-based rate limiting and request filtering (AS-08).
  \item \textbf{TLS termination} -- configure HTTPS at the reverse proxy for
        production deployment (ZTA tenet 2).
  \item \textbf{UUID primary keys} -- replace sequential integer IDs with UUIDs
        to eliminate the ID enumeration vector (AS-03).
  \item \textbf{Device posture and mTLS} -- require mutual TLS for service-to-service
        communication to meet ZTA tenets 1 and 2 fully.
\end{enumerate}
